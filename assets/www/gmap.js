var mapdata = { destination: new google.maps.LatLng(-1.645645, 36.845454) };
...
$('.refresh').live("tap", function() {
           ...
           // END: Tracking location with device geolocation
            ...
            if (toggleval) {
                toggleval = false;
                position = { coords: { latitude: -1.25874, longitude: 36.244884 } }; // Suswa
            } else {
                toggleval = true;
                position = { coords: { latitude: -2.54754, longitude: 36.874545 } }; // Somewhere Nairobi
            }
            $('#map_canvas').gmap('displayDirections', 
                { 'origin' : new google.maps.LatLng(position.coords.latitude, position.coords.longitude), 
